...
const CARGOS_PRIMERA_FILA = ["presidencia", "vicepresidencia", "secretaría", "prosecretaría"];
...
function MiembroCard({ item }: { item: any }) {
  ...
  const nombre = [diputado?.nombres, diputado?.apaterno, diputado?.amaterno].filter(Boolean).join(" ");
  ...
  return (
    <div className="integrante-comision">
      ...
      <div className="info-diputado-basica">
        <h4 className="nombre-cc-centrado"><strong>{nombre || "Sin nombre"}</strong></h4>
...
export default function ComisionPage({ comision }: { comision: any }) {
  ...
  const primeraFila = integrantes
    ...
    .sort((a, b) => getCargoPrioridad(a.tipo_cargo?.valor ?? "") - getCargoPrioridad(b.tipo_cargo?.valor ?? ""));
      ...
      <div className="body_comites">
        <div className="columns-20 w-row">
          <div className="w-col w-col-6">
            ...
            <h1 className="titulo-seccion">{comision.alias || comision.nombre}</h1>
            {comision.descripcion && <p className="texto-comunicado">{comision.descripcion}</p>}
          </div>
          <div className="column-26 w-col w-col-6">
            <img src="/images/placeholder-foto.jpg" loading="lazy" alt="" className="img_cc-hero" />
          </div>
        </div>
      </div>
            ...
            {tabActiva === "integrantes" && (
              <div className="w-tab-pane w--tab-active">
                ...
                <div className="w-layout-grid grid-integrante-cc">
                  {primeraFila.map((item) => <MiembroCard key={item.id} item={item} />)}
                  {segundaFila.map((item) => <MiembroCard key={item.id} item={item} />)}
                </div>
              </div>
            )}
