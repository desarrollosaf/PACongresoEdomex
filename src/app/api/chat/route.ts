...
toolChoice: 'auto' as const,
      system: `REGLA CERO - OBLIGATORIA ANTES DE CUALQUIER RESPUESTA:
JAMÁS respondas con datos de tu memoria o entrenamiento. 
SIEMPRE debes llamar la tool correspondiente ANTES de responder, 
sin excepción. Aunque creas saber la respuesta, DEBES consultar la tool  primero.
Si no hay tool  aplicable, entonces sí responde directamente.
      Eres el Asistente Virtual Oficial del Congreso del Estado de México. Actualmente estamos en la LXII Legislatura.
...
8. Si es sobre la Mesa Directiva (presidente, secretario, etc.), usa "obtener_mesa_directiva" y filtra por cargo en tu respuesta. 
...
20. Una vez que ya tienes los datos de una herramienta y puedes responder al usuario, 
DEBES responder en texto inmediatamente. NO llames otra herramienta si ya tienes 
la información necesaria para contestar.`,
      tools: {
        buscar_diputado: tool({
          ...
          execute: async (args) => {
            ...
            // Si busca "todos" o "listar" devuelve todos
        ...
        listar_comisiones: tool({
          ...
          execute: async () => {
            ...
            // Tu API devuelve { "Comisiones Legislativas": [...], "Comisiones Especiales": [...] }
            ...
            if (Array.isArray(res)) {
              ...
              // Aplanar todas las llaves del objeto
              Object.values(res).forEach((grupo: any) => {
                if (Array.isArray(grupo)) {
                  todasLasComisiones = [...todasLasComisiones, ...grupo];
                }
              });
            }
        ...
        buscar_sesion_agenda: tool({
          ...
          execute: async (args) => {
            ...
            // Los eventos tienen: descripcion (nombre del tipo), fecha_hora / fecha_hora_inicio, sede.sede
            const coincidencias = raw.filter((a: any) => {
              const descripcion = normalizar(a.descripcion || '');
              return descripcion.includes(tipoNorm);
            });

            if (coincidencias.length === 0) {
              // Devolver muestra de descripciones disponibles para diagnóstico
          ...
          execute: async (args) => {
            ...
            if (!id && nombre) {
                ...
                if (res) {
                    ...
                    // Estrategia 1: la BD contiene la query
                    let c = todasLasComisiones.find((x: any) => normalizar(x.nombre || x.alias || '').includes(queryNorm));

                    // Estrategia 2: la query contiene el nombre de la BD (nombre largo del usuario)
                    if (!c) c = todasLasComisiones.find((x: any) => queryNorm.includes(normalizar(x.nombre || x.alias || '')));

                    // Estrategia 3: todas las palabras clave de la query aparecen en el nombre de la BD
                    ...
                    // Estrategia 4: al menos la mitad de palabras clave coinciden (match parcial)
