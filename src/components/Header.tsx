...
export default function Header() {
  ...
  const isDesktop = () => typeof window !== 'undefined' && window.matchMedia('(hover: hover) and (min-width: 992px)').matches;

  const Dropdown = ({ id, label, children }: { id: DropdownKey; label: string; children: React.ReactNode }) => (
    <div
      className={`w-dropdown${open === id ? ' w--open' : ''}`}
      onMouseEnter={() => { if (isDesktop()) setOpen(id); }}
      onMouseLeave={() => { if (isDesktop()) setOpen(null); }}
    >
    ...
    <header className="section-2" ref={headerRef}>
      <div className="div-block-8">
        <div className="navbar w-nav">
          <Link href="/" aria-current="page" className="w-nav-brand">
            <img
          ...
          <div className="container-4 container-5 w-container">
            <nav className={`nav-menu w-nav-menu${menuOpen ? ' w--nav-menu-open' : ''}`}>
