...
export default function ClientShell({
  ...
  const isReconocimientoDocente2026 = pathname.startsWith("/ReconocimientoDocente2026");
  const isLandingStandalone = isConvocatoria || isReconocimiento || isParlamentojuvenil || isReconocimientoDocente2026;
  return (
    <>
      {!isLandingStandalone && <Header />}

      <main className="flex-1">
        {children}
      </main>

      {!isLandingStandalone && <Footer />}
      {!isLandingStandalone && <FeedbackWidget />}
